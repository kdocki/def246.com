...
function loadSprite(id, x, y)
{
  ...
  // Converting this to prototype because Weebly does not like jQuery  
  Event.observe(document, 'mousemove', animateSprites);
}
...
{
  try {
	if(oldClass[i] != "") jQuery(s).removeClassName(oldClass[i]);
	jQuery(s).addClassName(c);
	oldClass[i] = c;
  } catch(e) {
	// do nothing, ignore any exceptions here... 
  }
}
...
function getSpriteDegree(s, e, x, y)
{
	...
	if(cx == -1) cx = jQuery(s).cumulativeOffset().left + jQuery(s).getWidth() / 2;
	if(cy == -1) cy = jQuery(s).cumulativeOffset().top + jQuery(s).getHeight() / 2;
